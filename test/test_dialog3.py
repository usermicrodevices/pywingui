...
if has_pywingui:
	class ChoiceDialog(Dialog):
		...
		items = [
			...
			ComboBox(id = IDC_COMBO_BOX, rcPos = RECT(10, 95, 190, 50)),
			DefPushButton(title = 'OK', id = IDOK, rcPos = RECT(left = 30, top = 110, right = 50, bottom = 15)),
			PushButton(title = 'Cancel', id = IDCANCEL, rcPos = RECT(left = 130, top = 110, right = 50, bottom = 15)),
			]

		_dialog_template_ = DialogTemplate(
			title = 'Select video settings',
			style = DS_MODALFRAME|WS_POPUP|WS_CAPTION|WS_SYSMENU|DS_SETFONT,
			rcPos = RECT(left = 0, top = 0, right = 210, bottom = 125),
			items = items
			)
		...
		def event_combo(self, event):
			self.driver_type_from_combo = self.ctrl_ComboBox.GetCurrentSelection()
		event_combo = cmd_handler(IDC_COMBO_BOX)(event_combo)
		...
		def OnInitDialog(self, event):
			...
			self.ctrl_ComboBox = self.GetDlgItem(self.IDC_COMBO_BOX, comctl.ComboBox)
			self.ctrl_ComboBox.AddString('Null driver')
			self.ctrl_ComboBox.AddString('Software driver')
			self.ctrl_ComboBox.AddString('Burningsvideo driver')
			self.ctrl_ComboBox.AddString('DirectX 8 driver')
			self.ctrl_ComboBox.AddString('DirectX 9 driver')
			self.ctrl_ComboBox.AddString('OpenGL driver')
			self.ctrl_ComboBox.SetCurrentSelection()
...
if __name__ == '__main__':
	if has_pywingui:
		...
		print('driver type %d' % dialog.driver_type)
		print('full_screen %d' % dialog.full_screen)
		print('stencil_buffer %d' % dialog.stencil_buffer)
		print('vsync %d' % dialog.vsync)
		print('anti_alias %d' % dialog.anti_alias)
		print('with_alpha_channel %d' % dialog.with_alpha_channel)
		print('double_buffer %d' % dialog.double_buffer)
		print('driver type from combo %d' % dialog.driver_type_from_combo)
